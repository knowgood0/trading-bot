...
def webull_sdk_info():
    ...
    try:
        ...
        result["webull_module"] = getattr(
            webull,
            "__file__",
            None
        )

        result["webull_version"] = getattr(
            webull,
            "__version__",
            None
        )
...
def selected_account_test():
    return jsonify({
        "success": True,
        "environment": resolve_account().get(
            "environment",
            "PAPER"
        ),
...
def webull_option_position_test():
    ...
    return jsonify(
        get_webull_option_position(symbol)
    )
...
def order_detail_test():
    ...
    return jsonify(
        test_order_detail(client_order_id)
    )
