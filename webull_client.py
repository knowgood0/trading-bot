...
# IMPORTANT:
#
# The official Webull Python SDK contains a dedicated option-order method:
#
#     trade_client.order_v2.place_option(account_id, new_orders)
#
# Its bundled official option sample uses:
#
#     /openapi/trade/option/order/place
#
# and the leg's "symbol" is the UNDERLYING symbol (SPY), NOT the OCC option
# symbol (SPY260814C00777000).
#
# The OCC option symbol remains useful for market-data lookup and local
# position matching, but it is NOT sent as the order leg's symbol.
#
# We intentionally use the SDK's dedicated option endpoint here instead of
# the unified order_v3 endpoint that was producing HTTP 417.
# ---------------------------------------------------------------------------
...
def _webull_place_option_order(
    ...
    try:
        ...
        # This is intentionally the SDK's dedicated OPTION order shape.
        #
        # Do NOT put the OCC option symbol in legs[0]["symbol"].
        # Webull's option-order sample uses the underlying ticker there.
        option_order = {
        ...
        # rather than:
        #
        #   order_v3.place_order(...)
        #
        # The SDK's dedicated option request automatically uses the
        # option-order endpoint/category.
        response = _webull_call(
        ...
        return {
            ...
            "order_status": _extract_order_status(
                response_data
            ),
        }
...
def _extract_order_status(data):
    ...
    # Keep this deliberately conservative. We do not translate an API status
    # into "FILLED" unless Webull actually reports that.
    for key in (
        "status",
        "order_status",
        "orderStatus",
...
def test_order_detail(client_order_id):
    """
    Option order detail is queried through the SDK's V2 order-detail method,
    matching Webull's official option sample.
    """
    try:
        trade_client, _ = get_clients()
...
def paper_buy_spy(option_type="CALL"):
    ...
    # One SPY quote per alert, then one option-chain request.
    spy_price = extract_spy_price()
    ...
    # Submit the actual PAPER option order.
    ...
    # IMPORTANT:
    # We only create the local/Google OPEN trade after Webull accepts the
    # order request. "Accepted/submitted" is NOT represented as "FILLED".
    ...
    trade = {
        ...
        "client_order_id": order_result.get(
            "client_order_id"
        ),
    }
...
def paper_sell_spy():
    ...
    # Close local/Google records only after Webull accepts the SELL.
...
def test_options():
    ...
    # Preserve the deliberate spacing between chain calls.
    time.sleep(1.5)
