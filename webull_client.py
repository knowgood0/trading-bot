...
def _webull_place_option_order(option_symbol, option_type, side, quantity,
    ...
    """Place a SINGLE option order through Webull's unified order endpoint.

    Webull's current API requires instrument_type=OPTION plus an options leg.
    The order is deliberately LIMIT, using the premium observed immediately
    before the alert was processed. BUY orders use GTC; SELL orders use DAY.
    """
    try:
        trade_client, _ = get_clients()
        normalized_type = str(option_type).upper()
        ...
        # Current Webull docs: options support LIMIT and DAY/GTC; GTC is buy-side only.
        tif = "GTC" if normalized_side == "BUY" else "DAY"
